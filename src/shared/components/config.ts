...
export const config = {
  ...
  port,
  jwtSecret: process.env.JWT_SECRET_KEY || 'dev',
  rpc: {
    jwtSecret: process.env.JWT_SECRET_KEY || 'dev',
  ...
  redis: {
    ...
    url: process.env.REDIS_URL || 'redis://default:dev@localhost:6379/0'
  },
  db: {
    ...
    url: process.env.DATABASE_URL || 'mysql://root:dev@localhost:3306/bento?connection_limit=100'
  },
  upload: {
    type: 'local',
    path: 'uploads',
